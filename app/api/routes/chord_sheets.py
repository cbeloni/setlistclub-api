...
def _enrich_sheet(sheet: ChordSheet) -> ChordSheetOut:
    data = ChordSheetOut.model_validate(sheet)
...
def list_chord_sheets(
    ...
    query = db.query(ChordSheet).options(joinedload(ChordSheet.created_by)).filter(ChordSheet.is_private == False)
    if current_user:
    ...
    return [_enrich_sheet(s) for s in sheets]
...
async def list_recently_viewed(
    ...
    return [_enrich_sheet(s) for s in chord_sheets]
...
@router.get("/shared-with-me", response_model=list[ChordSheetOut])
